...
#' The volatility is calculated based on the square of day-to-day price changes (absolute value).
#'
...
comp_ewma_volatility <- function(stock_data, window = 36) {
  ...
  price_changes <- diff(Cl(stock_data))
  ...
  for(i in 1:nrow(stock_data)) {
    if(i <= 2) {
      variance[i] <- NA  # NA for the first two since diff reduces length by 1
    } else {
      variance[i] <- decay_factor * price_change_power_two[i-1] + (1 - decay_factor) * variance[i - 1]
    }
  }
